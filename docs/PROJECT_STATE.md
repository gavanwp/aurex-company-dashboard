...
- **Monorepo:** Turborepo + pnpm.
  ...
  - `supabase/migrations` — 18 SQL migrations (`0001`–`0018`). Enterprise RBAC ([ADR-0008](adr/0008_Enterprise_Identity_And_RBAC.md) / [EnterpriseIdentityAndRBAC.md](architecture/EnterpriseIdentityAndRBAC.md)): `0017` = **Organization tier** above workspaces (1:1 backfill, `create_workspace` updated, `workspaces.organization_id` NOT NULL). `0018` = **data-driven RBAC engine** (`permissions` catalog · `roles` · `role_permissions` · `user_permission_overrides` · `resource_grants` · `organization_policies`), seeded with **103 permissions + the 19 system roles + the matrix**, and `workspace_members.role_id` backfilled. Both steps are **no-behavior-change**: nothing reads the engine yet — the app still uses `packages/core/permissions` `can()` until the `0019` cutover. _DB types for the engine tables land with `0019` (their first consumer)._
