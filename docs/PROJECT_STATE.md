...
- **Monorepo:** Turborepo + pnpm.
  ...
  - `supabase/migrations` — 19 SQL migrations (`0001`–`0019`). Enterprise RBAC ([ADR-0008](adr/0008_Enterprise_Identity_And_RBAC.md) / [EnterpriseIdentityAndRBAC.md](architecture/EnterpriseIdentityAndRBAC.md)): `0017` = **Organization tier** (1:1 backfill, `workspaces.organization_id` NOT NULL). `0018` = **data-driven RBAC engine** (`permissions`/`roles`/`role_permissions`/`user_permission_overrides`/`resource_grants`/`organization_policies`) seeded with 103 permissions + 19 system roles + the matrix; `workspace_members.role_id` backfilled. `0019` = **cutover started**: `has_permission()` SQL resolver + `apps/web/lib/permissions.ts` (in-process `getEffectivePermissions`/`hasPermission`/`requirePermission`, DENY-wins, org-owner elevation). **The Team module is migrated to the engine as the proof** (verified: owner keeps manage + comp, employees restricted — behavior identical). _Remaining cutover: migrate the other module guards + retire the hardcoded `can()` + guard-test suite + JWT permission-digest fast path._
