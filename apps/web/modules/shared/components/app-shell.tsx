...
import { Sheet, SheetContent, SheetTitle } from '@aurexos/ui/components/sheet'
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
/**
 * The sidebar body — brand, nav, and footer. Shared by the desktop rail (which
 * can collapse to icons) and the mobile drawer (always labelled). `collapsed`
 * only ever true on desktop; `onNavigate` closes the mobile drawer on click.
 */
function SidebarBody({
  ...
  const labelClass = collapsed ? 'hidden' : 'truncate'
  const tipHidden = collapsed ? '' : 'hidden'
  ...
  return (
    <>
      ...
      <div className="flex h-14 shrink-0 items-center gap-2.5 px-3">
        ...
        <span className={cn('min-w-0 flex-col', collapsed ? 'hidden' : 'flex')}>
          <span className="truncate text-sm font-semibold text-foreground">AurexOS</span>
          <span className="truncate text-xs text-sidebar-foreground/60">AI operating system</span>
        </span>
      </div>
                ...
                <Tooltip key={item.label}>
                  <TooltipTrigger asChild>
                    <Link
                      ...
                      className={cn(
                        'relative flex h-9 items-center gap-2.5 rounded-md px-2.5 text-sm font-medium transition-colors md:h-8',
              ...
              <Tooltip key={item.label}>
                <TooltipTrigger asChild>
                  <div
                    aria-disabled="true"
                    tabIndex={0}
                    className="flex h-9 cursor-not-allowed items-center gap-2.5 rounded-md px-2.5 text-sm text-sidebar-foreground/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring md:h-8"
                  >
                    <item.icon className="size-5 shrink-0" aria-hidden="true" />
                    <span className={cn('flex-1', labelClass)}>{item.label}</span>
                    <span
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                ...
                <span className={cn('flex-1', labelClass)}>AI assistant</span>
      ...
      {/* Bottom: profile + settings + workspace card */}
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="flex h-svh w-full overflow-hidden bg-background text-foreground">
        ...
        >
          <SidebarBody pathname={pathname} workspace={workspace} collapsed={collapsed} />
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          {/* Top bar */}
          <header className="flex h-14 shrink-0 items-center gap-1 border-b border-border px-2 sm:gap-2 sm:px-4">
